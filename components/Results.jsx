...
const Results = ({ daysData }) => {
    ...
    const calculateAssignments = () => {
        ...
        const results = daysData.map((day) => {
            ...
            const sortedAgents = availableAgents.sort((a, b) => agentCounts[a] - agentCounts[b]);
    ...
    return (
        <div className="container">
            ...
            <button onClick={calculateAssignments}>Càlcul d'hores extra</button>
            <ul>
            ...
            <ul>
                {workerSchedule.map((worker, index) => (
                    <li key={index}>
                        {`Agent ${worker.Agent}: Dies - ${worker.Days}, Total Dies - ${worker.TotalDays}`}
                    </li>
                ))}
            </ul>
